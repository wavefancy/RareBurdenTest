...
# Using R SPAtest for binary phenotype.
...
*** Call R glm to do regression for continuous traits, to match the results of epacts,
*** Call SPAtest for binary phenotype, output summary for spline based meta analysis.
...
for line in eachline(stdin)
    ...
    if length(mytitle) == 0
        ...
        if data == nothing
            ...
            if f_rare == 0
                if SPAtest == true
                    out = vcat(repr(n_sample), ["0", "NA", "NA", "NA", "NA", "NA"])
                else
                ...
                if SPAtest == false
                    ...
                    if length(covars) == 0
                        ...
                        r = rcopy(R"ScoreTest_SPA_wMeta(score,tpheno,tcov,minmac=1,Cutoff=2,output='metaspline',beta.out=T, beta.Cutoff = $firthP)")
                        #r = rcopy(R"ScoreTest_SPA_wMeta(score,tpheno,tcov,minmac=1,Cutoff=2,output='metaZ',beta.out=T, beta.Cutoff = $firthP)")
                    end
                    # println(r)
                    # output is a dict
                    ...
                    spldata = r[:spldata]
                    out = vcat(repr(n_sample), [@sprintf("%.4E", f_rare), p, beta, sebeta, wald_p, join(spldata,",")])
                end
